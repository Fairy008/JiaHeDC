    ...
    var Method = {
        'default': function ($container, pluginName, methodName) {
            ...
            if (methodName == 'default') {
                return botDataTable.init($container, pluginName, opts);
            } else {
                if (this[methodName]) {
            ...
            botDataTable.refresh($container, pluginName);
        }
    };
    var botDataTable = {
        'init': function ($table, pluginName, opts) {
            $table.addClass('bot-data-table').wrap('<div class="bot-data-table-frame"></div>');
            var $frame = $table.parent('.bot-data-table-frame');
            var $tbody = $table.children('tbody');
            if (opts.botpaginate.enabled && opts.botpaginate.changeDisplayLen) {
                this.topBox.draw($frame);
                var $topBox = $frame.children('.data-table-top-box');
                this.changeDisplayLen.draw($topBox, opts.botpaginate.displayLen, opts.botpaginate.displayLenMenu);
            ...
            if (opts.showInfo || opts.botpaginate.enabled) {
                this.bottomBox.draw($frame);
                var $bottomBox = $frame.children('.data-table-bottom-obox');
                if (opts.showInfo) {
                    this.info.draw($bottomBox);
                }
                if (opts.botpaginate.enabled) {
                    this.botpaginate.init($frame, $table, $tbody, $bottomBox, pluginName, opts, this);
                }
            }
            ...
            'draw': function ($frame) {
                $frame.append('<div class="data-table-bottom-obox"></div>');
            }
        }, 'tbody': {
            ...
            'draw': function ($topBox, displayLen, displayLenMenu) {
                ...
                opt.botpaginate.displayLen = selected;
                opt.botpaginate.currentPage = 1;
            ...
            'init': function ($frame, $table, $tbody, $paginateFrame, pluginName, opts, top) {
                $paginateFrame.botpaginate({
                    'visibleGo': opts.botpaginate.visibleGo,
                    'type': opts.botpaginate.type,
                    'pageBtnFun': function (page) {
                        var cOpts = optHandle.get($table, pluginName);
                        cOpts.botpaginate.currentPage = page;
                        optHandle.set($table, pluginName, cOpts);
                        top.view($frame, $table, $tbody, pluginName, false);
                    }
                });
            }, 'refresh': function ($paginateFrame, opts, dataLen) {
                $paginateFrame.botpaginate('refreshPageBtn', {
                    'pageData': {
                        'currentPage': opts.botpaginate.currentPage,
                        'dataLen': dataLen,
                        'displayDataLen': opts.botpaginate.displayLen
                    }
                });
            }
            ...
            var currentPage = opts.botpaginate.currentPage;
            var displayDataLen = opts.botpaginate.displayLen;
            ...
            if (opts.showInfo) {
                var $info = $frame.children('.data-table-bottom-obox').children('.info-box');
                this.info.set($info, startNo, endNo, dataLen);
            }
            if (opts.botpaginate.enabled) {
                this.botpaginate.refresh($frame.children('.data-table-bottom-obox'), opts, dataLen);
            }
            opts.afterShow(opts.errFlag, opts.errMsg, dataLen, ajaxBackData);
        }, 'ajax': function (opts, callback) {
            var toData = {};
            if (opts.botpaginate.enabled) {
                toData.botpaginate = true;
                toData.currentPage = opts.botpaginate.currentPage;
                toData.displayDataLen = opts.botpaginate.displayLen;
            } else {
                toData.botpaginate = false;
            }
...
//          }
    ...
    $.fn.botDataTable = function (options, param) {

        var pluginName = 'botDataTable';
        options = options || {};
        var r = [];
        this.each(function () {
    ...
    $.fn.botDataTable.defaults.ajaxBack = {};
    $.fn.botDataTable.defaults.ajaxBack.defaults = {
        'errFlag': false,
        'errMsg': '',
        'dataLen': 0,
        'data': [],
