define([],function(){
	var formVerfication = {
		 ...
		 */
		checkDsInputLength : function(start,end,obj,showTargetObj,showInfo,showInfoTwo){
			var currentObj = this;
			clearErrorInfo(obj,showTargetObj);
			var val = obj.val();
			debugger
		...
		checkInputIsQQ : function(obj,showTargetObj,showInfo){
			var currentObj = this;
			clearErrorInfo(obj,showTargetObj);
			if(currentObj.isEmpty(obj.val())){//qq号可以为空
				return false;
		 ...
		 */
		checkInputIsCharDigitalUnderlineMiddleLine: function (obj, showTargetObj, showInfo) {
			var currentObj = this;
			clearErrorInfo(obj,showTargetObj);
		 ...
		 * @version <1> 2018-02-08 djh: created.
		 */
		checkInputLengthAndIsNull: function (start, end, obj, showTargetObj, showInfo, showNullInfo) {
			var currentObj = this;
			clearErrorInfo(obj, showTargetObj);
		 ...
		 * @version <1> 2018-4-8 lxy: created.
		 */
		checkInputIsCharDigitalUnderlineaAndDot: function (obj, showTargetObj, showInfo) {
			var currentObj = this;
			clearErrorInfo(obj,showTargetObj);
		...
		checkInputisRgbColor: function (obj, showTargetObj, showInfo) {
			var currentObj = this;
			clearErrorInfo(obj,showTargetObj);
			if(!currentObj.isRgbColor(obj.val().trim())){
				obj.addClass('formInputHighlight');
				showTargetObj.append(getHtml(showInfo));
				return true;
			}
		...
		checkInputIsPlusMinusNumber : function(obj,showTargetObj,showInfo){
			...
			if(!currentObj.isPlusMinusInteger(obj.val().trim()) && !currentObj.isPlusMinusDouble(obj.val().trim())){
				obj.addClass('formInputHighlight');
				showTargetObj.append(getHtml(showInfo));
				return true;
			}
