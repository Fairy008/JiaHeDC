define(["jquery","BaseAjax","custom_settings","formVerfication","PopWin","Base64"],function($,BaseAjax,custom_settings,formVerfication,PopWin,Base64){
    var commons = {
        /**
         * 用户退出登录
        ...
        quitFun : function(){
            ...
            ajax.opts.successFun = function(msg){
                ...
                commons.delCookie(COOKIE_CONFIG.cookieName)
                var exp = new Date();
            ...
            ajax.opts.successFun = function (msg){
                if(msg.flag){
                    if(msg.data != null ){
                        ...
                        // console.log("8989898989898989898",data.accountId)
                        // $(".userCenter .user").html(commons.queryAccountName(data.accountId));
                        var result =commons.isLogion();
                        $(".userCenter .user").html(JSON.parse(result.data).nickName||JSON.parse(result.data).personName||JSON.parse(result.data).accountName||JSON.parse(result.data).accountCode);
                        ...
                        // $('#myHomeBtn').attr('href','personal.html?accountId='+data.accountId);
         ...
         * 进入页面时，判断用户是否具有后台管理权限
         * @version<1>2019-03-18 lijie : created.
         */
        isCmsFun : function(){
            var ajax = new BaseAjax();
            ajax.opts.type = "GET";
            ajax.opts.url = CMS_MANAGE.findUserCms_url+"?accountId="+$(".user").attr("accountId");
            ajax.opts.successFun = function (msg){
                if(msg.flag){
         ...
         */
        menuShow : function(roleType){

            var obj = this;
            var ajax = new BaseAjax();
            ajax.opts.url = LOGIN_CONFIG.findForumMenu_url + "?roleType=" + roleType ;
            ajax.opts.successFun = function (msg) {
                if(msg.flag){
                    var result = msg.data;
                    var html ="<ul>";
        ...
        */
         ...
         */
        findDictList:function(parentId,selectId,id,msg){
            var ajax = new BaseAjax();
        ...
        showTaskType:function(selectId){
            var task_types=custom_settings.task_types;
            $("#"+selectId+" option").each(function(){
                var thisObj=$(this)
                if(thisObj.val()!='' && task_types.indexOf(thisObj.val())==-1){
                    thisObj.hide();
                }
            });
        },
        loginAndRegister:function(){
            /*重置且清空注册登录框*/
            var resetFun = function(current){
                //重置
                $('#loginError,#errorInfo,#updError').text('');//提示信息
                $('#accountName,#accountPwd,#code').val('');//手机号或用户名，登录密码，图形验证码
                $('#pnumber,#pwd,#pvalicode').val('');//手机号,设置密码，手机验证码
                $('#phone,#validCode,#newPwd,#reNewPwd').val('');//手机号 验证码  新密码 确认新密码
                $('.popBox').hide();
                //删除重复元素
                $.each(current.parents('.popBox'),function (i,n) {
                    if (i>0) n.remove();
                });
                $('body').append(current.parents('.popBox').clone(true));
                //避免删除唯一一个元素
                if (current.parents('.popBox').length>1){
                    $('#menuInfo').empty();
                }
                $('#menuInfo').hide();
            ...
            /**
             * 注册
             * @version<1> 2019-3-14 lml: Created.
             */
            $("#register,#registerBtn").click(function() {
                resetFun($(this));
                //删除重复元素
                $.each($('.popBox.registerBox'),function (i,n) {
                    if (i>0) n.remove();
                });
                $('.popBox.registerBox').appendTo($('#menuInfo'));
                $('#menuInfo .popBox.registerBox').show();
                $('#menuInfo').show();
            });

            $("#login,.clickLogin,#hasPadLogin,#reLogin,.clickLoginBtn").click(function() {
                resetFun($(this));
                //删除重复元素
                $.each($('.popBox.loginBox'),function (i,n) {
                    if (i>0) n.remove();
                });
                $('.popBox.loginBox').appendTo($('#menuInfo'));
                $('#menuInfo .popBox.loginBox').show();
                $('#menuInfo').show();
            });

            $("#forgetPwd,#forgetPwdBtn").click(function() {
                resetFun($(this));
                //删除重复元素
                $.each($('.popBox.forgetPwdBox'),function (i,n) {
                    if (i>0) n.remove();
                });
                $('.popBox.forgetPwdBox').appendTo($('#menuInfo'));
                $('#menuInfo .popBox.forgetPwdBox').show();
                $('#menuInfo').show();
            });
        ...
        /**
         * 检查门户用户是否登录， 若未登录，则跳转至登录页面
         * @returns {boolean}
         * @version<1> 2019-03-11 lcw:Created.
         */
        checkForumLoginStatus:function(){
         ...
         * @version <1> 2019/3/19 mason:Created.
         */
        findExpertByAccountId : function (accountId) {
            var user = new Object();
            var ajax = new BaseAjax();
            ajax.opts.url = FORUM_EXPERT.nolog_findExpertByAccountId_url+"?accountId="+accountId;
            ajax.opts.type = 'get';
            ajax.opts.async = false;
            ajax.opts.successFun = function (result) {
                //专家信息
                if (result.flag && null != result.data){
                    ...
                    user.photoUrl = expert.photoUrl;
                    user.userName = expert.expertName;
                    user.introduction = expert.introduction;
                    user.industry = expert.industry;
                    user.expertId = expert.expertId;
                    user.industryCode = commons.getIndustryColor(expert.industry);
                }
            };
            ajax.opts.errorFun = function () {
        ...
        /**
         * 根据用户accountId查询该用户用户表信息
         * @param
         * @return
         * @version <1> 2019/3/19 mason:Created.
         */
        getUserByAccountId :function (accountId) {
            var user = new Object();
            var ajax = new BaseAjax();
            ...
            ajax.opts.successFun = function(result){
                if(result.flag && result.data != null){
                    var permPerson = result.data;
                    user.userName = permPerson.personName
                    user.introduction = permPerson.remark==null?'':permPerson.remark;
                    user.photoUrl = permPerson.photoUrl;
                }
         ...
         * @version <1> 2019/3/19 mason:Created.
         */
        getIndustryColor : function (industry) {
        ...
        /**
         * 提示框遮罩层
         * @param message(消息内容) time(消失时间ms) falg(成功true,失败false)
         * @return 
         * @version <1> 2019/3/20 15:36 zhangshen:Created.
         */
        tipMessageShow : function (message, time, falg) {
            if (falg) {
                $("body").append('<div class="overlay"></div><div class="prompt borderSuccess"><i class="iconSuccess"></i><span class="tipMessage"></span></div>');
            } else {
                $("body").append('<div class="overlay"></div><div class="prompt borderFail"><i class="iconFail"></i><span class="tipMessage"></span></div>');
            }
            $(".tipMessage").html(message);
            $(".prompt").show().delay(time).fadeOut(function(){$(this).remove()});
            $(".overlay").show().delay(time).fadeOut(function(){$(this).remove()});
        }
