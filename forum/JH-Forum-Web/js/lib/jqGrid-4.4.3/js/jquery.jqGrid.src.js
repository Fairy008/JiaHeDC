 ...
 * Date: 2013-01-21
 * @version <2>修改高度为自适应高度  lcw 2017-12-15
 */
//jsHint options
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			url: "",
			height: '100%',  //修改高度为自适应高度  lcw 2017-12-15
			page: 1,
			rowNum: 20,
			rowTotal : null,
			records: 0,
			...
			loadComplete: null,
			gridComplete: null,
			loadError: function(data){//登录失效后, 点jqGrid查询会报6000或6001错误, 登录失效跳转登录页面
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				if(found && p.tmplFilters && p.tmplFilters.length) {
					$(".ui-briefing", fil).bind('change', function(){
