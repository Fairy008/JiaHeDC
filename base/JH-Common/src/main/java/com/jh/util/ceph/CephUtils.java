...
//        return PropertyUtil.getPropertiesForConfig("CEPH_ROOT", CEPH_CONFIG).replace("\\",File.separator);
    ...
    public static String getShowPath1(String path){

        return getCephRoot().replace("/", File.separator) + path.replace("/" , File.separator);
    }
...
//
    ...
    private static boolean scanDirectory(String path)  {
        ...
        if(!file.exists()){
            bool = file.mkdirs();
            /*try {
                Runtime.getRuntime().exec("sudo chmod -R 775 "+ path.replace("\\",File.separator));
                logger.info("权限修改成功 --------------- chmod -R 775 "+path);
            }
            catch (Exception e)
            {
                e.printStackTrace();
                logger.info("权限修改失败 --------------- chmod -R 775 "+   e.getMessage());
            }*/
...
//    }
     ...
     * @param
     * @return url 图片地址
     * @version <1> 2019/8/21 sxj:Created.
     */
    public static String uploadAdvertImage(HttpServletRequest request) {
        MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
        ...
        String reportStorage = CephUtils.getAbsolutePath("").replace("\\", File.separator);
        //图片目录
        String photoUrl=CephUtils.getCephUrl("FORUM_ADVERT_PIC_STORAGE").replace("\\",File.separator)+File.separator;
        ...
        String suff = "jpeg";//文件后缀,默认使用jpg
        //加时间戳，防止文件同名，浏览器缓存问题
        String newName="photo_url_" + "_"+ new Date().getTime() + "." + suff;//重新生成用户名，防止中文名
        String storagePath = reportStorage +photoUrl + newName;;
        File copyFile = new File(storagePath);//文件
        try {
            file.transferTo(copyFile);//复制文件
