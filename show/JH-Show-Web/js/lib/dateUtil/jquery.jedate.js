...
;(function(root, factory) {
    ...
    jedfn.setValue = function (fnStr,matStr,bool) {
        ...
        if(vals=='undefined-undefined-undefined')
        {
            vals = (this.areaVal)[0];
        }
    ...
    jedfn.renderHtml = function(ys, ms, ds, opts,boxcls){
        ...
        var timeStr = function () {
            ...
            /*<span class="clear">'+clearTxt+'</span><span class="today">'+lang.today+'</span><span class="setok">'+lang.yes+'</span>*/
            btnStr = '<span class="today">'+lang.today+'</span><span class="setok">'+lang.yes+'</span>',
    ...
    jedfn.eachYM = function(y, m,opts,boxCell,clsCell) {
        ...
        $.each(new Array(multiPane ? 1 : 2),function (s) {
            ...
            $.each(formatYY ? yearArr : lang.month, function (n, val) {
                var ym = s==1 ? parseInt(y) + (formatYY ? yearArr.length : 1) : y,seCls;
        ...
        tdCls.on("click", function(ev) {
            ...
            var areaSelected = function () {
                ...
                if (that.areaStart && that.areaEnd == undefined){
                    ...
                    var s1 = new Date(that.areaVal[0]),
                        s2=new Date(that.areaVal[1]);
                    if(opts.sameDate == "year"){
                        if(s1.getFullYear() != s2.getFullYear()){
                            cleanArea();
                        } else {
                            heightShow();
                        }
                    } else if(opts.sameDate == "month"){
                    ...
                    // if(opts.isSameMonth){ //遍历元素，并在范围中查找同时着色
                    //     var s1 = new Date(that.areaVal[0]),
                    //         s2=new Date(that.areaVal[1]),
                    //         total = (s2.getTime() - s1.getTime())/1000,
                    //         day = parseInt(total / (24*60*60));//计算整数天数
                    //     day = day>=0 ? day : -day;
                    //     var day_month = jet.getDaysNum(s1.getFullYear(),s1.getMonth()+1);
                    //     if(s1.getFullYear()==s2.getFullYear()&& s1.getMonth() == s2.getMonth()) {
                    //         heightShow();
                    //     } else{
                    //         cleanArea();
...
/*            var areaSelected = function () {
                //判断是否只选中一个
                if (that.areaStart && that.areaEnd){
                    //如果已经选择了一个范围，就清除属性
                    that.delAreaAttr();
                    tdCls.removeClass(carr[0]).removeClass(carr[1]);
                    lithis.addClass(carr[0]);
                    ...
                    if (that.areaStart && that.areaEnd == undefined){
                        lithis.addClass(carr[0]);
                        that.areaEnd = true;
                        thisdate = addDate(new Date(thisdate),29);
                        if(thisdate.substring(0,4)!=tt.substring(0,4)) thisdate=tt.substring(0,4)+"-12-31";
                        if(thisdate.substring(5,10)==tt.substring(5,10)) thisdate=tt.substring(0,4)+"-12-02";
                        //添加当前选中的到数组中
                        that.areaVal.push(thisdate);
                        //遍历元素，并在范围中查找同时着色
                        tdCls.each(function () {
                            ...
                            if (!sefl.hasClass("other") && !sefl.hasClass("disabled")){
                                ...
                                if(parseInt(seVals) > parseInt(minVal) && parseInt(seVals) ==parseInt(maxVal)){
                                    sefl.addClass(carr[0]);
                                }
                                if(seVals==thisdate.replace(/-/g,"")){
                                    sefl.addClass(carr[0]);
                                }
                            }
                        });
    ...
    jedfn.clickBtn = function (opts,boxCell) {
        ...
        boxCell.on("click",".setok", function(ev) {
            ...
            var temp = true;
            if (isShow) {
                if (okVal.DD == '' || okVal.DD == null || okVal.DD == 'undefined') {
                    temp = false;
                }
            ...
            if (temp){
                if (opts.range == false) {
                    ...
                    /*   if(objVal.length==1) {
                     var tar = tm.split('-');
                     objVal.push({'YYYY': tar[0], 'MM': tar[1], 'DD': tar[2]});
                     }*/
                }
                if ($.isFunction(opts.okfun) || opts.okfun != null) {

                    opts.okfun({elem:elemCell,val:valdate,date:objVal});
                }
            }
            else {
                that.dateClose();
