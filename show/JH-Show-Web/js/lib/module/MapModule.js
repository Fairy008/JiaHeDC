...
//		     if(!_this.opts.logoHref) _this.opts.logoHref="";
		...
		*/
         ...
         * ==================================================================================================
         *
         * The Overlay To show Selected Patch
         *
         * ==================================================================================================
         */
        var _highlight;//高亮度显示的feature
        var _featureOverlay;

        /**
         * HighLine feature on specified vector layer when mouse pointer on it .
         * @param evt
         * @param vector_layer_name
         */
        this.highFeatureOverlay = function (pixel, vector_layer_name,style) {
            if(!style) style=MapTool.defineStyle({fillColor:"rgba(225, 225, 225, 0.5)", strokeColor:"#f6f1eb", strokeWidth:1});
            if (!(_featureOverlay instanceof ol.layer.Vector)) {
                _featureOverlay = new ol.layer.Vector({
                    source: new ol.source.Vector(),
                    style: style,
                    zIndex:999
                });
                _this.map.addLayer(_featureOverlay);
            }

            var feature = _this.getFeatureByClick(pixel, vector_layer_name);

            if (feature != _highlight && _featureOverlay instanceof ol.layer.Vector) {
                if (_highlight && (_highlight instanceof ol.Feature)) {
                    _featureOverlay.getSource().removeFeature(_highlight);
                }
                if (feature) {
                    _featureOverlay.getSource().addFeature(feature);
                }
                _highlight = feature;
            }
...
//              gfLayer.setVisible(false);
		...
		*/
		...
		this.repaintLegend = function(legend){
			...
			divLegend.style="left:117px;bottom:80px;background-color: rgba(255,255,255,.8);"
