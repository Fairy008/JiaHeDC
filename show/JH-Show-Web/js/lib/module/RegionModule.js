    ...
    function RegionSelector(targetId,opts){
        ...
        */
        ...
        var __loadRegion = function(parentRegionId){
            ...
            var dataCategoryVal = $("#selDataCategory").val();
            if(dataCategoryVal == 2){
                ajax.opts.data = {parentId:parentRegionId,dataType:dataCategoryVal};
            }else{
                ajax.opts.data = {parentId:parentRegionId};
            }
            ajax.opts.async = false;
            ajax.opts.type = "post";
            ajax.opts.contentType = 'application/x-www-form-urlencoded'
