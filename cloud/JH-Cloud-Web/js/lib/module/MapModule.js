	...
	function MapTool(targetId,opts){
		...
		*/
		var __init = function(){
		    ...
		    if(!_this.opts.zoom) _this.opts.zoom=5;
		    if(!_this.opts.minZoom) _this.opts.minZoom=0;
		    if(!_this.opts.maxZoom) _this.opts.maxZoom=18;
		    // if(!_this.opts.logoSrc) _this.opts.logoSrc="../../images/public/logo.png";
		    // if(!_this.opts.logoHref) _this.opts.logoHref="http://www.datall.cn";
		...
		*/
		var colorLayer;
		this.addColorLayer = function(showLayer,lvStyle,lvDate,zIndex){
			...
			colorLayer = new ol.layer.Vector({
				...
				style:function (feature,resolution) {
					var REGION_ID = feature.get('REGION_ID');
					if(lvDate[REGION_ID] && lvDate[REGION_ID]['LV']){
						feature.setStyle(lvStyle[lvDate[REGION_ID]['LV']]);
					}
			...
			showLayer.getSource().getSource().forEachFeature(function(feature){
		...
		* @version <1> 2017-11-08 Hayden:Created.
		*/
		this.highFeature = function(feature,highStyle){
			if(!highStyle) highStyle = MapTool.defineStyle({fillColor:"rgba(255,215,0, 0.1)",strokeColor:"#20B2AA"});
			if(highFeature && highFeature!=feature) highFeature.setStyle(null);
			feature.setStyle(highStyle);
			highFeature = feature;
		...
		* @version <1> 2017-11-08 Hayden:Created.
		*/
		this.moveToLayerFeature = function(layer,fieldName,fieldValue){
			var selectedFeature ;
			var source ;
			...
			if(source){
				...
				if(selectedFeature){
					...
					var point = ol.extent.getCenter(geom.getExtent());
	       			...
	       			_this.highFeature(selectedFeature);
		...
		*/
		var __addDefaultControl =  function(_flagControl){
			...
			var zoomElemBar = document.getElementsByClassName("ol-zoom")[0];
			zoomElemBar.style="top:10px;right:10px;left:auto;"
		...
		var __addCustomControl = function(zoomElemBar){
			var btn = document.createElement("button");
			btn.title="选择";
			btn.innerHTML="<img src='images/map/sicon3.png' width='20' height='20'/>";
			zoomElemBar.appendChild(btn);
			...
			btn.title="全景";
			btn.innerHTML="<img src='images/map/sicon6.png' width='20' height='20'/>";
			zoomElemBar.appendChild(btn);
			btn.onclick = function(){
			...
			btn.id="btnLength";
			btn.title="测长度";
			btn.innerHTML="<img src='images/map/sicon5.png' width='20' height='20'/>";
			zoomElemBar.appendChild(btn);
			btn.onclick = function(){
			...
			btn.id="btnArea";
			btn.title="测面积";
			btn.innerHTML="<img src='images/map/sicon4.png' width='20' height='20'/>";
			zoomElemBar.appendChild(btn);
			btn.onclick = function(){
            ...
            btn.id="btnShowGF";
            btn.title="高清底图";
            btn.innerHTML="<img src='images/map/sicon7.png' width='20' height='20'/>";
            zoomElemBar.appendChild(btn);
            $(btn).toggle(function () {
		...
		*/
		...
		this.repaintLegend = function(legend){
			...
			divLegend.style="right:10px;bottom:30px;background-color: rgba(255,255,255,.8);"
			...
			for(var i=0;i<legend.length;i++){
				...
				spanValue.style="margin:2px 10px;display:inline-block;font-size:80%;width:35px;";
